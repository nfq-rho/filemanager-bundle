...
var nfqFileMangerDialog = {
    ...
    inProgress : function() {
        ...
        this.timeoutStore = window.setTimeout(function(){
            document.getElementById("upload_additional_info").innerHTML = 'This is taking longer than usual.' + '<br />' + 'An error may have occurred.' + '<br /><a href="#" onClick="nfqFileMangerDialog.showIframe()">' + 'View script\'s output' + '</a>';
...
$(document).ready(function() {
    $('.image-delete').on('click', function() {
        ...
        if (r == true) {
            $.ajax({
                type: "POST",
                url: $('#image-delete-url').val(),
                data: { data: $(this).parent().find('img').attr('src') }
            });
            $(this).parent().remove();
        }
    });
    $('.image-item .image').on('click', function() {
        var image = $(this).find('img').attr('src');

        window.nfqFileMangerDialog.uploadFinish({
            filename: image,
